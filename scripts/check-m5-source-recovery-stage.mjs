...
if (parents(head).length !== 1 || parents(head)[0] !== M5_RUNTIME_AUTHORIZATION_COMMIT ||
    !matchesExpectedRows(commitRows(head), M5_RUNPOD_ENV_RECOVERY_EXPECTED) ||
  ...
  throw new Error("M5 append-only RunPod environment recovery stage is not exact");
}
assertM5WorktreeExact();
if (!existsSync(M5_RUN_AUTHORIZATION_PATH) || !existsSync(M5_RUNTIME_AUTHORIZATION_PATH)) {
  throw new Error("M5 RunPod environment recovery lost an inherited authorization receipt");
}
for (const forbidden of [M5_RUNPOD_ENV_AUTHORIZATION_PATH, M5_SELECTION_LOCK_PATH, M5_FAILURE_PATH, M5_LARGE_SOURCE_LOCK_PATH, M5_FINAL_RECEIPT_PATH, "docs/COMPETITOR_AUDIT.md"]) {
  if (existsSync(forbidden)) throw new Error(`M5 RunPod environment recovery contains forbidden later evidence: ${forbidden}`);
}
console.log(JSON.stringify({ head, parent: M5_RUNTIME_AUTHORIZATION_COMMIT, priorAuthorization: M5_RUNTIME_AUTHORIZATION_COMMIT, policy: "pass" }));
