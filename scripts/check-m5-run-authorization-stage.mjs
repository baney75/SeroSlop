...
if (head !== result.authorization || result.receipt.status !== "m5-parity-recovery-authorized") {
  throw new Error("M5 authorized stage must be the exact A5 parity receipt commit");
}
...
for (const forbidden of [M5_SELECTION_LOCK_PATH, M5_FAILURE_PATH, M5_LARGE_SOURCE_LOCK_PATH, M5_FINAL_RECEIPT_PATH, "docs/COMPETITOR_AUDIT.md"]) {
  if (existsSync(forbidden)) throw new Error(`M5 parity authorization contains forbidden later evidence: ${forbidden}`);
}
...
if (publicHead !== head) throw new Error("M5 parity authorization must be the anonymous public main head");
