...
import { M5_A6_STATUS, M5_A7_STATUS, M5_FAILURE_PATH, M5_FINAL_RECEIPT_PATH, M5_LARGE_SOURCE_LOCK_PATH, M5_SELECTION_LOCK_PATH } from "./m5-stage-policy.mjs";
...
if (existsSync("benchmark/evidence/m5/failure-replay-recovery-authorization.json")) {
  const result = validateM5FailureVerifierChain();
  if (head !== result.authorization || result.receipt.status !== M5_A7_STATUS) throw new Error("M5 authorized stage must be the exact A7 failure-replay receipt commit");
  assertM5WorktreeExact();
  for (const forbidden of [M5_SELECTION_LOCK_PATH, M5_LARGE_SOURCE_LOCK_PATH, M5_FINAL_RECEIPT_PATH, "docs/COMPETITOR_AUDIT.md"]) if (existsSync(forbidden)) throw new Error("M5 A7 cannot authorize success evidence");
  const publicReference = await getJson("https://api.github.com/repos/baney75/prooflens/git/ref/heads/main");
  if (publicReference.object?.sha !== head) throw new Error("M5 A7 authorization must be anonymous public main");
  ...
  if (sourceRun.head_sha !== result.source || sourceRun.event !== "push" || sourceRun.status !== "completed" || sourceRun.conclusion !== "success" || sourceRun.path !== ".github/workflows/quality.yml" || sourceRun.html_url !== result.receipt.sourcePublicCi.url) throw new Error("M5 R7 source CI proof does not match public GitHub state");
