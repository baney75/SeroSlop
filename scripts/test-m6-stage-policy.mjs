...
import {
  ...
  M6_SUBMISSION_PROXY_R3_EXPECTED, M6_SUBMISSION_PROXY_R3_STATUS, M6_SUBMISSION_PROXY_R3_COMMIT,
  M6_SUBMISSION_PROXY_R3_TREE, M6_SUBMISSION_PROXY_R3_ARTIFACT_SHA256,
  M6_SUBMISSION_PROXY_RESULT_EXPECTED, M6_SUBMISSION_PROXY_RESULT_STATUS,
  M6_SUBMISSION_PROXY_RESULT_ARTIFACT_SHA256,
...
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_R_COMMIT}^{tree}`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_R_TREE);
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_R_COMMIT}^`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_S_COMMIT);
const submissionProxyRecoveryRows = M6_SUBMISSION_PROXY_R2_EXPECTED.map(([path, status]) => [path, status]);
assert.equal(M6_SUBMISSION_PROXY_R2_STATUS, "m2-bounty-proxy-ci-recovery-ready");
assert.equal(matchesM6SubmissionProxyR2Head({ head: "e".repeat(40), parent: M6_SUBMISSION_PROXY_R_COMMIT, rows: submissionProxyRecoveryRows }), true);
assert.equal(matchesM6SubmissionProxyR2Head({ head: "e".repeat(40), parent: M6_SUBMISSION_PROXY_S_COMMIT, rows: submissionProxyRecoveryRows }), false);
assert.equal(matchesM6SubmissionProxyR2Head({ head: "e".repeat(40), parent: M6_SUBMISSION_PROXY_R_COMMIT, rows: submissionProxyRecoveryRows.slice(1) }), false);
assert.equal(matchesM6SubmissionProxyR2Head({ head: "e".repeat(40), parent: M6_SUBMISSION_PROXY_R_COMMIT, rows: [...submissionProxyRecoveryRows, ["extra", "A"]] }), false);
assert.deepEqual(Object.fromEntries(Object.keys(M6_SUBMISSION_PROXY_R2_ARTIFACT_SHA256).map((path) => [
  path,
...
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_R3_COMMIT}^{tree}`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_R3_TREE);
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_R3_COMMIT}^`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_R2_COMMIT);
const submissionProxyResultRows = M6_SUBMISSION_PROXY_RESULT_EXPECTED.map(([path, status]) => [path, status]);
assert.equal(M6_SUBMISSION_PROXY_RESULT_STATUS, "m2-bounty-proxy-result-published");
assert.equal(matchesM6SubmissionProxyResultHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: submissionProxyResultRows }), true);
assert.equal(matchesM6SubmissionProxyResultHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R2_COMMIT, rows: submissionProxyResultRows }), false);
assert.equal(matchesM6SubmissionProxyResultHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: submissionProxyResultRows.slice(1) }), false);
assert.equal(matchesM6SubmissionProxyResultHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: [...submissionProxyResultRows, ["extra", "A"]] }), false);
assert.equal(matchesM6SubmissionProxyResultHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: submissionProxyResultRows.map(([path, status], index) => [path, index === 0 ? "A" : status]) }), false);
assert.deepEqual(Object.fromEntries(Object.keys(M6_SUBMISSION_PROXY_RESULT_ARTIFACT_SHA256).map((path) => [
  path,
  createHash("sha256").update(m5GitBytes(["show", `${M6_SUBMISSION_PROXY_RESULT_COMMIT}:${path}`])).digest("hex"),
])), M6_SUBMISSION_PROXY_RESULT_ARTIFACT_SHA256);
assert.equal(isM6ProtocolLineageHead({ head: "a".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: submissionProxyResultRows }), true);
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_RESULT_COMMIT}^{tree}`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_RESULT_TREE);
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_PROXY_RESULT_COMMIT}^`]).toString("utf8").trim(), M6_SUBMISSION_PROXY_R3_COMMIT);
const submissionProxyResultRecoveryRows = M6_SUBMISSION_PROXY_RESULT_RECOVERY_EXPECTED.map(([path, status]) => [path, status]);
assert.equal(M6_SUBMISSION_PROXY_RESULT_RECOVERY_STATUS, "m2-bounty-proxy-result-ci-recovery-ready");
assert.equal(matchesM6SubmissionProxyResultRecoveryHead({ head: "b".repeat(40), parent: M6_SUBMISSION_PROXY_RESULT_COMMIT, rows: submissionProxyResultRecoveryRows }), true);
assert.equal(matchesM6SubmissionProxyResultRecoveryHead({ head: "b".repeat(40), parent: M6_SUBMISSION_PROXY_R3_COMMIT, rows: submissionProxyResultRecoveryRows }), false);
assert.equal(matchesM6SubmissionProxyResultRecoveryHead({ head: "b".repeat(40), parent: M6_SUBMISSION_PROXY_RESULT_COMMIT, rows: submissionProxyResultRecoveryRows.slice(1) }), false);
assert.equal(matchesM6SubmissionProxyResultRecoveryHead({ head: "b".repeat(40), parent: M6_SUBMISSION_PROXY_RESULT_COMMIT, rows: [...submissionProxyResultRecoveryRows, ["extra", "A"]] }), false);
assert.deepEqual(Object.fromEntries(Object.keys(M6_SUBMISSION_PROXY_RESULT_RECOVERY_ARTIFACT_SHA256).map((path) => [
  path,
  createHash("sha256").update(m5GitBytes(["show", `${M6_SUBMISSION_RELEASE_PARENT}:${path}`])).digest("hex"),
])), M6_SUBMISSION_PROXY_RESULT_RECOVERY_ARTIFACT_SHA256);
assert.equal(isM6ProtocolLineageHead({ head: "b".repeat(40), parent: M6_SUBMISSION_PROXY_RESULT_COMMIT, rows: submissionProxyResultRecoveryRows }), true);
const submissionReleaseRows = M6_SUBMISSION_RELEASE_EXPECTED.map(([path, status]) => [path, status]);
assert.equal(M6_SUBMISSION_RELEASE_STATUS, "m2-submission-release-candidate-unverified");
assert.equal(m5GitBytes(["rev-parse", `${M6_SUBMISSION_RELEASE_PARENT}^{tree}`]).toString("utf8").trim(), M6_SUBMISSION_RELEASE_PARENT_TREE);
assert.equal(matchesM6SubmissionReleaseHead({ head: "c".repeat(40), parent: M6_SUBMISSION_RELEASE_PARENT, rows: submissionReleaseRows }), true);
assert.equal(matchesM6SubmissionReleaseHead({ head: "c".repeat(40), parent: M6_SUBMISSION_PROXY_RESULT_COMMIT, rows: submissionReleaseRows }), false);
assert.equal(matchesM6SubmissionReleaseHead({ head: "c".repeat(40), parent: M6_SUBMISSION_RELEASE_PARENT, rows: submissionReleaseRows.slice(1) }), false);
assert.equal(matchesM6SubmissionReleaseHead({ head: "c".repeat(40), parent: M6_SUBMISSION_RELEASE_PARENT, rows: [...submissionReleaseRows, ["extra", "A"]] }), false);
assert.equal(matchesM6SubmissionReleaseHead({ head: "c".repeat(40), parent: M6_SUBMISSION_RELEASE_PARENT, rows: submissionReleaseRows.map(([path, status], index) => [path, index === 0 ? "A" : status]) }), false);
assert.deepEqual(Object.fromEntries(Object.keys(M6_SUBMISSION_RELEASE_ARTIFACT_SHA256).map((path) => [
  path,
  createHash("sha256").update(readFileSync(path)).digest("hex"),
])), M6_SUBMISSION_RELEASE_ARTIFACT_SHA256);
...
assert.equal(staticRouter.includes("matchesM6SubmissionProxyResultHead"), true);
assert.equal(staticRouter.includes("matchesM6SubmissionProxyResultRecoveryHead"), true);
assert.equal(staticRouter.includes("matchesM6SubmissionReleaseHead"), true);
assert.equal(staticRouter.includes('npm", ["run", "verify:submission-release"]'), true);
