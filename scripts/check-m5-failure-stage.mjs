...
function rows(commit) {
  return git(["diff-tree", "--root", "--no-renames", "--name-status", "--format=", "-r", commit])
    .split("\n").filter(Boolean).map((line) => {
      ...
      if (!status || !pathname || extra !== undefined) throw new Error(`Malformed M5 failure row: ${line}`);
...
execFileSync("python3", ["-c", [
  ...
  "from benchmark.m5.contracts import load_recipe,parse_json_bytes,read_jsonl,validate_failure_receipt",
  "r=load_recipe(Path('benchmark/m5/recipe.json'))",
  `f=parse_json_bytes(Path('${M5_FAILURE_PATH}').read_bytes(),label='failure receipt')`,
