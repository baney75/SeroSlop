...
|---|---|---|---|
| `pick` | `Choose an image` | Analyze one user-selected image and nothing else. | `Choose image` |
...
The selected mode is global and persists locally. The setup page presents the three radio choices only after `Offline ready`, followed by one explicit `Save mode` action. The popup shows the active host, current mode, page counts, one `Change` action, and one mode-specific primary action. Its inline mode sheet uses the same names and descriptions as setup and does not save merely because a radio receives focus. A failed save restores the previous mode and keeps the sheet open with a short error.
