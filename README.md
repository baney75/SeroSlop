...
The score-blind replacement packet has not been scored. It contains 300 Coxy7 Infinity images and 300 KoalaAI StockImages-CC0 photographs. Its 319-row false-positive slice uses different StockImages rows and bytes, but the same source corpus, so it is not an independent source-population estimate. The fixed dataset cards report CC BY 4.0 and CC0-1.0 respectively; those are source-reported license statements, not independent rights clearance. The public V2 receipt records the boundary before the failed first run. A new binary64 recovery source and receipt-only V3 freeze must both be public and green before the replacement evaluator may load ONNX Runtime, Pillow, model bytes, or pixels. [BENCHMARK.md](BENCHMARK.md) records the append-only lineage, exact replacement hashes, gates, and limits.
