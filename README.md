...
Public source commit `0771a9422b552e2023e5150fb6c8b4238b811a74` passed its pre-score checks. Its receipt-only child `2bd0c4757f6059c57879414a5dba77629d66460e` did not: the legacy Node verifier buffered the 87,442,080-byte model through the default child-process limit and stopped with `ENOBUFS`. That failure occurred before the evaluator ran, and the canonical confirmatory and web-negative output directories remain absent. The legacy receipt stays unchanged for audit. A narrowly allowlisted recovery source and a new receipt-only `pre-score-freeze-v2.json` commit must both pass locally, on public GitHub Actions, and through anonymous exact-head verification before sealed inference is allowed. Repository history proves that no canonical sealed output was committed before the recovery; it cannot prove that no pixels were viewed outside this recorded workflow.
