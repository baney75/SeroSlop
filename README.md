...
The validation-only balanced accuracy is 95.00% on originals, 96.33% on screenshots, and 94.50% on both JPEG stress views. These are model-selection results, not the final estimate. The shipped model has not scored the 600-image confirmatory set. Publication is blocked unless every view clears the predeclared lower-95% gates for balanced accuracy and both class recalls, and unless the separate real-image challenge clears its false-positive gates. [BENCHMARK.md](BENCHMARK.md) freezes the exact splits, hashes, statistical methods, commands, post-score policy, and limitations before any confirmatory result is observed.

Public source commit `0771a9422b552e2023e5150fb6c8b4238b811a74` passed its pre-score checks. Its receipt-only child `2bd0c4757f6059c57879414a5dba77629d66460e` did not: the legacy Node verifier buffered the 87,442,080-byte model through the default child-process limit and stopped with `ENOBUFS`. Recovery source `99861df575854511c685d7b8f90acdc7ed4e5923` fixed that bug, but its [public CI run](https://github.com/baney75/prooflens/actions/runs/31846361076) exposed a test-only ordering flaw: the copied evaluator imported inference dependencies that the pixel-free CI environment intentionally does not install, so the test could not reach the new authorization guard. Both failures occurred before the evaluator scored anything; no V2 receipt or canonical confirmatory/web-negative output exists. The legacy receipt and both failed commits stay unchanged for audit. A nine-path second recovery source and the first receipt-only `pre-score-freeze-v2.json` commit must both pass locally, on public GitHub Actions, and through anonymous exact-head verification before sealed inference is allowed. Repository history proves that no canonical sealed output was committed before this recovery; it cannot prove that no pixels were viewed outside the recorded workflow.
