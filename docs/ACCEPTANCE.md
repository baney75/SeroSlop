...
|---|---|---|
| MIT source | Root `LICENSE`; upstream/runtime attribution | `npm run check:package` and repository audit |
...
| Automatic ordinary-page analysis | `<img>`, `currentSrc`, `picture/srcset`, dynamic DOM, CSS backgrounds; local viewport crop when canvas is blocked | Chrome E2E target assertions |
...
| ≥75% balanced accuracy | 93.83% original sealed holdout; lower 95% CI 91.83% | `benchmark/results/` and `BENCHMARK.md` |
